function outputData = ASL_model(ustar, ~, L, z_i, z0)
...
for p = 1:np
    ...
    while in_domain
        ...
        % If particle leaves x bounds or top of BL, mark it as
        % "not in domain", which starts the next particle.
        ...
        elseif z <= zmin
            ...
            [sigu, sigw, dsigu2, dsigw2, uw, duw, tau, ubar, dt]...
                = ASL_windstats(z,L,ustar,z0,v_s);
            
            [a_u, a_w, b_u, b_w] = ASL_coeffs(sigu, sigw,...
                dsigu2,dsigw2, uw, duw, tau, up, wp);
            
            dup = a_u*dt + b_u*randn*sqrt(dt);
            dwp = a_w*dt + b_w*randn*sqrt(dt);
...
rogueVels = 999;
outputData = {[rogueVels,r_crit,r_critdep,percdep,np,...
